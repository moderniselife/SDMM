/**
 * AllMedia Page — Browse the entire media library (all sources) with filters and pagination.
 */
import { FilterBar } from '@/components/common/FilterBar';
import { MediaGrid } from '@/components/media/MediaGrid';
import { Button } from '@/components/ui/button';
import { ChevronLeft, ChevronRight, AlertCircle, Library } from 'lucide-react';
...
export function AllMedia() {
  ...
  const { data, loading, error } = useApi(
    () =>
      fetchMedia({
        source, // All Media page — respect the filter bar's source selection
        resolution,
        codec,
        status,
        sort,
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-xl font-bold text-foreground">All Media</h1>
          <p className="text-xs text-muted-foreground">
            Everything across local, RealDebrid &amp; TorBox
          </p>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center py-16 text-muted-foreground">
          <Library className="mb-3 h-8 w-8 opacity-40" />
